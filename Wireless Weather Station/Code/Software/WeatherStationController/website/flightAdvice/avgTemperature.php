...
$sql = "SELECT * from averagesensordata where stationid='1' and sensorid='T' order by ReadingHour desc limit 12;";
     ...
     while($row = $result->fetch_assoc()) {
	    ...
	    $SensorId = $row["SensorId"];
     	if( $SensorId == "T" && $StationId == "1") {
	         ...
	         $tempArr = array('', $ReadingHour.'.00', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot2Average"];
	         $tempArr = array('', $ReadingHour.'.05', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot3Average"];
	         $tempArr = array('', $ReadingHour.'.10', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot4Average"];
	         $tempArr = array('', $ReadingHour.'.15', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot5Average"];
	         $tempArr = array('', $ReadingHour.'.20', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot6Average"];
	         $tempArr = array('', $ReadingHour.'.25', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot7Average"];
	         $tempArr = array('', $ReadingHour.'.30', $avg);
			 array_push($data, $tempArr);  // populates $data array
	         ...
	         $tempArr = array('', $ReadingHour.'.45', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot11Average"];
	         $tempArr = array('', $ReadingHour.'.50', $avg);
			 array_push($data, $tempArr);  // populates $data array

	         $avg = $row["Slot12Average"];
	         $tempArr = array('', $ReadingHour.'.55', $avg);
			 array_push($data, $tempArr);  // populates $data array
     	}
     }
...
# Main plot title:
$plot->SetTitle('Average ** Temparature Readings every hour:'.date("h:i:sa"));
